...
export default function App() {
  ...
  const saveMovement = () => {
    ...
    setMovements((items) => [{ id: Date.now(), product: product.name, productId: id, type: dialog, quantity: qty, employee: form.employee.trim(), note: form.note.trim(), date: new Date().toLocaleString("he-IL") }, ...items]);
    setDialog(null);
    notify(dialog === "in" ? "המלאי נקלט בהצלחה" : "המלאי הוצא בהצלחה");
  };

  const addProduct = () => {
    if (!newItem.name.trim() || !newItem.barcode.trim()) return notify("יש למלא שם מוצר וברקוד");
    if (products.some((p) => p.barcode === newItem.barcode.trim())) return notify("הברקוד כבר קיים במערכת");
    const product = { id: Date.now(), name: newItem.name.trim(), barcode: newItem.barcode.trim(), category: newItem.category, stock: Math.max(0, Number(newItem.stock) || 0), minStock: Math.max(0, Number(newItem.minStock) || 0) };
    ...
    <div dir="rtl" className="min-h-screen bg-slate-100 text-slate-900">
      <header className="bg-slate-950 text-white shadow-lg">
        <div className="mx-auto flex max-w-6xl items-center justify-between gap-4 px-4 py-5">
          <div className="flex items-center gap-3"><div className="rounded-2xl bg-blue-600 p-3"><Package size={25} /></div><div><h1 className="text-xl font-bold sm:text-2xl">ניהול מלאי ציוד משרדי</h1><p className="text-sm text-slate-300">מחסן הרצליה</p></div></div>
      ...
      <main className="mx-auto max-w-6xl px-4 py-6">
        ...
        {tab === "inventory" && <><div className="relative mt-5"><Search className="absolute right-4 top-3.5 text-slate-400" size={20} /><input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="חיפוש לפי מוצר, קטגוריה או ברקוד" className="w-full rounded-2xl border border-slate-200 bg-white py-3 pr-12 pl-4 outline-none focus:border-blue-500 focus:ring-2 focus:ring-blue-100" /></div><div className="mt-4 grid gap-3">{filtered.map((p) => <ProductCard key={p.id} product={p} onIn={() => openMovement("in", p.id)} onOut={() => openMovement("out", p.id)} onDelete={() => removeProduct(p.id)} />)}{!filtered.length && <Empty text="לא נמצאו מוצרים" />}</div></>}
        ...
        {tab === "history" && <div className="mt-5 overflow-hidden rounded-2xl bg-white shadow-sm">{movements.map((m) => <div key={m.id} className="flex flex-col gap-2 border-b border-slate-100 p-4 last:border-0 sm:flex-row sm:items-center sm:justify-between"><div><div className="flex items-center gap-2"><span className={`rounded-full px-2.5 py-1 text-xs font-bold ${m.type === "in" ? "bg-emerald-100 text-emerald-700" : "bg-rose-100 text-rose-700"}`}>{m.type === "in" ? "קליטה" : "הוצאה"}</span><strong>{m.product}</strong></div><p className="mt-1 text-sm text-slate-500">{m.employee}{m.note ? ` · ${m.note}` : ""}</p></div><div className="text-left"><strong className={m.type === "in" ? "text-emerald-600" : "text-rose-600"}>{m.type === "in" ? "+" : "-"}{m.quantity}</strong><p className="text-xs text-slate-400">{m.date}</p></div></div>)}{!movements.length && <Empty text="עדיין אין תנועות מלאי" />}</div>}
      </main>

      {dialog && <Modal title={dialog === "new" ? "הוספת מוצר חדש" : dialog === "in" ? "קליטת מלאי" : "הוצאת מלאי"} onClose={() => setDialog(null)}>{dialog === "new" ? <div className="grid gap-4"><Field label="שם המוצר"><input value={newItem.name} onChange={(e) => setNewItem({ ...newItem, name: e.target.value })} className="input" placeholder="לדוגמה: עט כדורי כחול" /></Field><Field label="ברקוד"><input value={newItem.barcode} onChange={(e) => setNewItem({ ...newItem, barcode: e.target.value })} className="input" placeholder="סריקה או הקלדה" /></Field><Field label="קטגוריה"><select value={newItem.category} onChange={(e) => setNewItem({ ...newItem, category: e.target.value })} className="input"><option>כלי כתיבה</option><option>מחברות ונייר</option><option>סוללות</option><option>אביזרי שולחן</option><option>אחר</option></select></Field><div className="grid grid-cols-2 gap-3"><Field label="מלאי התחלתי"><input type="number" min="0" value={newItem.stock} onChange={(e) => setNewItem({ ...newItem, stock: e.target.value })} className="input" /></Field><Field label="סף מלאי נמוך"><input type="number" min="0" value={newItem.minStock} onChange={(e) => setNewItem({ ...newItem, minStock: e.target.value })} className="input" /></Field></div><button onClick={addProduct} className="rounded-xl bg-blue-600 px-4 py-3 font-bold text-white hover:bg-blue-700">שמור מוצר</button></div> : <div className="grid gap-4"><Field label="מוצר"><select value={form.productId} onChange={(e) => setForm({ ...form, productId: e.target.value })} className="input"><option value="">בחר מוצר</option>{products.map((p) => <option key={p.id} value={p.id}>{p.name} · במלאי {p.stock}</option>)}</select></Field><Field label="כמות"><input type="number" min="1" value={form.quantity} onChange={(e) => setForm({ ...form, quantity: e.target.value })} className="input" /></Field><Field label="שם העובד"><input value={form.employee} onChange={(e) => setForm({ ...form, employee: e.target.value })} className="input" placeholder="מי ביצע את הפעולה?" /></Field><Field label="הערה"><input value={form.note} onChange={(e) => setForm({ ...form, note: e.target.value })} className="input" placeholder="מיקום, מחלקה או סיבה" /></Field><button onClick={saveMovement} className={`rounded-xl px-4 py-3 font-bold text-white ${dialog === "in" ? "bg-emerald-600 hover:bg-emerald-700" : "bg-rose-600 hover:bg-rose-700"}`}>אישור הפעולה</button></div>}</Modal>}
...
function Tab({ active, onClick, icon, text }) {
  return <button onClick={onClick} className={`flex flex-1 items-center justify-center gap-2 rounded-xl px-3 py-3 text-sm font-bold transition ${active ? "bg-slate-950 text-white" : "text-slate-500 hover:bg-slate-100"}`}>{icon}{text}</button>;
}
